...
# Define sweep parameters dynamically based on BATCH_SIZE
aer_vals = np.linspace(1.0, 20.0, BATCH_SIZE)             
vmax_multipliers = np.linspace(1.0, 500.0, BATCH_SIZE)
...
# ── 2. Execution Loop ────────────────────────────────────────────────────────
for i, aer_val in enumerate(aer_vals):
    # Set the initial aerobic biomass before setting up physics
    inflow.aer = aer_val
    
    # Initialise physics with the current batch size
    device, state = setup_physics(cfg)
    
    vmax_tensor = torch.tensor(vmax_multipliers, dtype=torch.float32, device=device).view(BATCH_SIZE, 1, 1)
    
    # Apply multiplier to all functional groups
    ...
    for n in tqdm(range(n_steps), desc=f"Sweep Row {i+1}/{BATCH_SIZE} (aer={aer_val:.1f})"):
        current_time = n * dt
        poc_t = poc_initial * torch.exp(-k_hyd * current_time)
        doc_flux_t = k_hyd * poc_t
        ...
        rhs_w, rhs_t = get_rhs_batched(w, tracers, psi_tot, state, cfg, doc_flux_t)
        ...
        rhs_w, rhs_t = get_rhs_batched(w1, t1, psi_tot, state, cfg, doc_flux_t)
        ...
        rhs_w, rhs_t = get_rhs_batched(w2, t2, psi_tot, state, cfg, doc_flux_t)
        ...
        # Sum biomass across all amplified functional groups for total community growth
        ...
        # Track the O2 and DOC Monod minima SEPARATELY (same core-mask + spatial min()
        # as above, just not combined via monod_min first). Note min_monod_batch above
        # is mathematically equivalent to torch.minimum(min_o2_monod_batch, min_doc_monod_batch)
        # since min() commutes -- so comparing these two tells us which substrate is actually
        # responsible for the recorded overall minimum, without needing extra bookkeeping.
        ...
        realized_growth = applied_vmax * monod_min * total_biomass
        ...
        # Peak average growth across the entire core
    ...
    for b in range(BATCH_SIZE):
        csv_data.append({
            'Initial_Aerobic_Biomass': float(aer_val),
            'Vmax_Multiplier': float(vmax_multipliers[b]),
            'Min_O2': float(results_min_o2[i, b]),
            'Max_Anoxic_Frac': float(results_anoxic_frac[i, b]),
            'Peak_Realized_Growth_day': float(results_realized_growth[i, b]),
            'Avg_Realized_Growth_day': float(results_avg_growth[i, b]),
    ...
    pd.DataFrame(csv_data).to_csv('run_amplification_results.csv', index=False)

# ── 3. Plotting: Lowest O2 ───────────────────────────────────────────────────
plt.figure(figsize=(9, 7))
X, Y = np.meshgrid(vmax_multipliers, aer_vals)
...
plt.xlabel('$V_{max}$ Multiplier (All Functional Groups)', fontsize=12)
plt.ylabel('Initial Aerobic Biomass (mmol C/m³)', fontsize=12)
plt.tight_layout()
plt.savefig('run_amplification_experiments.png', dpi=300)
...
plt.xlabel('$V_{max}$ Multiplier', fontsize=12)
plt.ylabel('Initial Aerobic Biomass (mmol C/m³)', fontsize=12)
plt.tight_layout()
plt.savefig('run_amplification_anoxic_frac.png', dpi=300)

# ── 5. Plotting: Realized Growth Rates (day⁻¹) ───────────────────────────────
plt.figure(figsize=(9, 7))

cp3 = plt.contourf(X, Y, results_realized_growth, levels=30, cmap='viridis')
cbar3 = plt.colorbar(cp3)
cbar3.set_label('Peak Realized Community Growth Rate (mmol C m$^{-3}$ day$^{-1}$)', fontsize=12)
...
plt.title('Parameter Sweep: Peak Local Realized Bulk Growth', fontsize=14, fontweight='bold')
plt.xlabel('$V_{max}$ Multiplier', fontsize=12)
plt.ylabel('Initial Aerobic Biomass (mmol C/m³)', fontsize=12)
plt.tight_layout()
plt.savefig('run_amplification_realized_rates.png', dpi=300)

# ── 6. Plotting: Average Realized Growth Rate (day⁻¹) ────────────────────────
plt.figure(figsize=(9, 7))

cp4 = plt.contourf(X, Y, results_avg_growth, levels=30, cmap='plasma')
cbar4 = plt.colorbar(cp4)
cbar4.set_label('Peak Average Core Growth Rate (mmol C m$^{-3}$ day$^{-1}$)', fontsize=12)
...
plt.title('Parameter Sweep: Peak Average Core Growth (Assimilation)', fontsize=14, fontweight='bold')
plt.xlabel('$V_{max}$ Multiplier', fontsize=12)
plt.ylabel('Initial Aerobic Biomass (mmol C/m³)', fontsize=12)
plt.tight_layout()
plt.savefig('run_amplification_avg_growth.png', dpi=300)
...
plt.ylabel('Initial Aerobic Biomass (mmol C/m³)', fontsize=12)
plt.tight_layout()
plt.savefig('run_amplification_peak_biomass.png', dpi=300)
...
plt.ylabel('Initial Aerobic Biomass (mmol C/m³)', fontsize=12)
plt.tight_layout()
plt.savefig('run_amplification_min_o2_monod.png', dpi=300)
...
plt.ylabel('Initial Aerobic Biomass (mmol C/m³)', fontsize=12)
plt.tight_layout()
plt.savefig('run_amplification_min_doc_monod.png', dpi=300)

# ── 8c. Plotting: Which Substrate Actually Drives the Minimum ────────────────
# Note: results_min_monod (the old combined "lowest Monod factor") is mathematically
# equivalent to np.minimum(results_min_o2_monod, results_min_doc_monod), since min()
# commutes with min(). So we don't need any new tracked state to see WHICH substrate
# is responsible for the overall limitation at each (aer, vmax) point -- we just
# compare the two arrays above. Positive = O2 is the tighter constraint (smaller
# factor); negative = DOC is the tighter constraint.
monod_dominance = results_min_doc_monod - results_min_o2_monod  # >0: O2-limited, <0: DOC-limited
...
plt.ylabel('Initial Aerobic Biomass (mmol C/m³)', fontsize=12)
plt.tight_layout()
plt.savefig('run_amplification_monod_dominance.png', dpi=300)
...
plt.ylabel('Initial Aerobic Biomass (mmol C/m³)', fontsize=12)
plt.tight_layout()
plt.savefig('run_amplification_limiting_factor.png', dpi=300)
